...
function authedService({ email = 'user@example.com', verified = true } = {}) {
  return {
    me: jest.fn().mockResolvedValue({
      user: { id: 1, email, emailVerified: verified, notificationPrefs: {} },
    }),
    login: jest.fn(),
...
describe('Account page — vault status card', () => {
  ...
  test(
    ...
    async () => {
      ...
      const { blob, saltV, data } = await makeEncryptedBlobFor({
        password,
        email,
      });
      const vaultService = {
